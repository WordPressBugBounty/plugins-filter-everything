...
class EntityManager
{
    ...
    private $allConfiguredFilters;

    public function __construct()
    {
        $this->allConfiguredFilters = $this->getAllConfiguredFilters();
    }
    ...
    */
    ...
    public function getFilterByEname( $e_name, $sets = [] )
    {
        ...
        foreach ( $actualSets as $actualSet ) {
            $key = $e_name . '_'.$actualSet['ID'];
            if ( isset( $this->allConfiguredFilters[$key] ) ){
                $theFilter = $this->allConfiguredFilters[$key];
                break;
            }
        }
    ...
    public function getFilterById( $filter_id )
    {
        ...
        $theFilter = [];
        foreach( $this->allConfiguredFilters as $filter ){
...
//        }
    ...
    public function getAllConfiguredFilters()
    {
        ...
        if( ! $filters = $this->getData( $key ) ){
            ...
            foreach( $this->makeFiltersQuery() as $k => $filter_post ){
                ...
                $k = $filter['e_name'].'_'.$filter['parent'];
                $filters[$k] = $filter;
            }
    ...
    */
    ...
    public function getConfiguredPathSlugs( $filters = NULL )
    {
        ...
        if( ! $filters ){
            $filters = $this->allConfiguredFilters;
        }
        ...
        if( ! $container->getParam( $key ) ) {
            ...
            foreach ( $allEntities as $entityName => $entity ) {
                if ( $entityName === '_stock_status' && ! $filter_by_stock_exists && $post_type === 'product' ) {
                    ...
                    $filter = $this->getFilterByEname( $entityName, $relatedSets );
                }
